  ...
  if (isBootstrapping) {
    return (
      <main className="dataflow-bg grid min-h-screen place-items-center px-6">
        <div className="dataflow-panel rounded-lg px-5 py-4 font-mono text-sm text-emerald-200">
          Carregando sessao...
        </div>
      </main>
    );
        ...
        <header className="mb-6 flex flex-col gap-4 border-b border-emerald-400/20 pb-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="h-10 rounded-md border border-slate-700 bg-slate-950/70 px-4 text-sm font-semibold text-slate-200 transition hover:border-emerald-400/60 hover:bg-emerald-400/10"
            >
              Sair
            </button>
          </div>
...
function LogoMark({ size = "md" }: { size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div
      className={`${boxSize} grid place-items-center rounded-xl border border-emerald-300/30 bg-slate-950/70 p-1 shadow-[0_0_32px_rgba(52,211,153,0.18)]`}
    >
...
function BrandIntro() {
  return (
    <div>
      <div className="inline-flex items-center gap-4 rounded-2xl border border-emerald-300/20 bg-slate-950/50 p-3 pr-6 shadow-[0_0_44px_rgba(52,211,153,0.14)]">
        <LogoMark size="lg" />
        <div>
          <p className="font-mono text-xs font-semibold uppercase tracking-[0.36em] text-emerald-300">Plataforma</p>
          <p className="mt-1 text-3xl font-black tracking-tight text-slate-50 sm:text-4xl">DataFlow</p>
        </div>
      </div>

      <h1 className="mt-8 max-w-3xl text-4xl font-semibold leading-tight text-slate-50 sm:text-5xl">
      ...
      <div className="dataflow-terminal mt-8 max-w-2xl rounded-lg p-4 font-mono text-sm text-slate-300">
        <div className="mb-4 flex items-center gap-2 border-b border-emerald-400/10 pb-3">
          ...
          <span className="ml-2 text-xs text-slate-500">pipeline.preview</span>
        </div>
        <p>
          <span className="text-emerald-300">$</span> upload clientes.csv
        </p>
    ...
    <form onSubmit={onSubmit} className="dataflow-panel relative overflow-hidden rounded-2xl p-6">
      ...
      <div className="mb-5 grid grid-cols-2 rounded-md border border-slate-700 bg-slate-950/70 p-1">
        <button
          ...
          className={`h-10 rounded px-3 text-sm font-semibold ${mode === "login" ? "bg-emerald-400 text-slate-950" : "text-slate-400 hover:text-slate-100"}`}
        >
          Entrar
        </button>
        <button
          ...
          className={`h-10 rounded px-3 text-sm font-semibold ${mode === "register" ? "bg-emerald-400 text-slate-950" : "text-slate-400 hover:text-slate-100"}`}
        >
          Criar usuario
        </button>
      </div>
      ...
      <input
        ...
        className="mt-2 h-11 w-full rounded-md border border-slate-700 bg-slate-950/80 px-3 font-mono text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-emerald-400"
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="mt-5 inline-flex h-11 w-full items-center justify-center rounded-md bg-emerald-400 px-5 text-sm font-bold text-slate-950 shadow-[0_0_24px_rgba(52,211,153,0.18)] transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
      >
  ...
  return (
    <aside className="dataflow-panel rounded-lg p-4 xl:sticky xl:top-6 xl:max-h-[calc(100vh-48px)] xl:overflow-auto">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-100">Historico</h2>
          <p className="mt-1 font-mono text-xs text-slate-500">{jobs.length} jobs encontrados</p>
        </div>
        <button
          type="button"
          onClick={onRefresh}
          className="h-9 rounded-md border border-slate-700 bg-slate-950/70 px-3 text-xs font-semibold text-slate-300 transition hover:border-emerald-400/60 hover:text-emerald-200"
        >
          {isLoading ? "..." : "Atualizar"}
        </button>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {jobs.length === 0 ? (
          <div className="rounded-md border border-dashed border-slate-700 bg-slate-950/50 p-4 text-sm text-slate-500">
    ...
    <form onSubmit={onUpload} className="dataflow-panel flex flex-col rounded-lg p-6">
      ...
      <div className="dataflow-terminal mt-5 flex min-h-44 flex-col items-center justify-center rounded-lg border-dashed px-4 py-8 text-center">
        <input
          ...
          className="w-full max-w-sm cursor-pointer rounded-md border border-slate-700 bg-slate-950/90 text-sm text-slate-300 file:mr-4 file:border-0 file:bg-emerald-400 file:px-4 file:py-3 file:text-sm file:font-bold file:text-slate-950 hover:file:bg-emerald-300"
        />
      </div>

      <div className="mt-5 rounded-lg border border-slate-800 bg-slate-950/50 p-4">
        <p className="font-mono text-xs font-semibold uppercase tracking-wide text-slate-500">Selecionado</p>
        <p className="mt-2 break-words text-sm font-medium text-slate-100">
          {selectedFile ? selectedFile.name : "Nenhum arquivo selecionado"}
        </p>
        {selectedFile ? <p className="mt-1 font-mono text-sm text-emerald-300">{formatBytes(selectedFile.size)}</p> : null}
      </div>

      <button
        type="submit"
        disabled={!canUpload}
        className="mt-6 inline-flex h-11 items-center justify-center rounded-md bg-emerald-400 px-5 text-sm font-bold text-slate-950 shadow-[0_0_24px_rgba(52,211,153,0.18)] transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
      >
...
function JobPanel({
  ...
  return (
    <aside className="dataflow-panel rounded-lg p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">Job atual</h2>
          <p className="mt-1 text-sm text-slate-500">Acompanhe o processamento e abra resultados do historico.</p>
        </div>
      ...
      {uploadResult || jobStatus ? (
        <button
          ...
          className="mt-6 inline-flex h-10 w-full items-center justify-center rounded-md border border-slate-700 bg-slate-950/70 px-4 text-sm font-semibold text-slate-200 transition hover:border-emerald-400/60 hover:bg-emerald-400/10 disabled:cursor-not-allowed disabled:text-slate-600"
        >
  ...
  return (
    <section className="dataflow-panel rounded-lg p-6">
      <div className="flex flex-col gap-2 border-b border-slate-800 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">Artefatos do processamento</h2>
          <p className="mt-1 text-sm text-slate-500">Script, manual e resumo da analise retornados pelo backend.</p>
        </div>
        <StatusBadge status={job.status} />
      </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-2">
          ...
          {job.result_package_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-11 items-center justify-center rounded-md border border-emerald-400/60 bg-emerald-400/10 px-4 text-sm font-semibold text-emerald-200 transition hover:bg-emerald-400/20 xl:col-span-2"
            >
              Abrir pacote ZIP
            </a>
          ) : null}
  ...
  return (
    <div className="mt-6 grid gap-4 lg:grid-cols-[240px_1fr]">
      ...
      <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-4">
        <h3 className="text-sm font-semibold text-slate-100">Inconsistencias detectadas</h3>
        {issues.length > 0 ? (
          <ul className="mt-3 space-y-3">
            {issues.map((issue, index) => (
              <li key={`${issue.type ?? "issue"}-${index}`} className="rounded-md border border-slate-800 bg-slate-900/70 p-3">
          ...
          <p className="mt-3 text-sm text-slate-500">Nenhuma inconsistencia foi retornada na analise.</p>
        )}
      </div>
    </div>
  );
...
function ArtifactViewer({
  ...
  return (
    <div className="rounded-lg border border-emerald-400/20 bg-slate-950">
      <div className="flex flex-col gap-3 border-b border-emerald-400/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="font-mono text-sm font-semibold text-emerald-200">{title}</h3>
        <div className="flex flex-wrap gap-2">
          {actions.map((action) => (
            <button
              ...
              className="rounded-md border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-100 transition hover:border-emerald-400/60 hover:bg-emerald-400/10"
            >
              {action.label}
            </button>
          ))}
...
function FailedState({ errorMessage }: { errorMessage?: string | null }) {
  return (
    <div className="mt-6 rounded-lg border border-red-400/30 bg-red-950/40 p-4 text-sm text-red-100">
...
function EmptyArtifacts() {
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-4 text-sm text-slate-500 xl:col-span-2">
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-4">
      <p className="font-mono text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p>
...
function StatusTimeline({ currentStatus }: { currentStatus: string }) {
  ...
  return (
    <div className="mt-6">
      <p className="font-mono text-xs font-semibold uppercase tracking-wide text-slate-500">Pipeline</p>
...
function ResultRow({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="font-mono text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</dt>
...
function getStatusClasses(status: string): string {
  ...
  if (status === "ANALYZING" || status === "SENT_TO_N8N") {
    return "border-sky-400/30 bg-sky-950/40 text-sky-100";
  }
